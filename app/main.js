...
const { computeAbsolutePath, tokenizeCommand, groupTokens } = require('./utility')
...
async function mainFn(words, stdin, isFinalCommand = false){
  ...
  let outputFd = 0
  const outputIdx = words.findIndex(word => word === '>' || word === '1>' || word === '>>' || word === '1>>')
  if(outputIdx > -1){
    const outputFile = computeAbsolutePath(words[outputIdx+1])

    if(words[outputIdx] === '>' || words[outputIdx] === '1>') { outputFd = fs.openSync(outputFile, "w"); } 
    else { outputFd = fs.openSync(outputFile, "a"); }
  ...
  if(errorIdx > -1){
    ...
    if(words[errorIdx] === '2>') { errorFd = fs.openSync(errorFile, "w"); }
    else { errorFd = fs.openSync(errorFile, "a"); }
  ...
  switch (words[0]) {
    ...
    case 'cd':
      ...
      if (fs.existsSync(absolutePath)) { process.chdir(absolutePath) }
    ...
    case 'echo':
      ...
      break;
    case 'type':
      if(BUILTIN_COMMANDS.includes(words[1])) { logger.log(`${words[1]} is a shell builtin`, out) }
      else{
        const result = words[1] in EXTERNAL_COMMANDS

        if (result) { logger.log(`${words[1]} is ${EXTERNAL_COMMANDS[words[1]]}`, out) } 
        else { logger.error(`${words[1]}: not found`, errorFd) }
      }
      break;
    case 'history':
    ...
    default:
      ...
      if(result){
        try{
          const spawnOptions = { stdio: ["pipe", (isFinalCommand ? "inherit": "pipe"), "inherit"], cwd: process.cwd() };

          if (outputFd) { spawnOptions.stdio[1] = outputFd; }
          if (errorFd) { spawnOptions.stdio[2] = errorFd; }
          ...
          if (outputFd) { fs.closeSync(spawnOptions.stdio[1]); }
          if (errorFd) { fs.closeSync(spawnOptions.stdio[2]); } 
